...
class Model:
    ...
    def loadData(self, split, seed, sub, imageHeight, imageWidth, batchSize):
        return tf.keras.preprocessing.image_dataset_from_directory(
            "/home/mir/Desktop/PokemonProject/greyscaleSet/archive/pokemon",
    ...
    def createModel(self):
        ...
        model.add(tf.keras.layers.Conv2D(16,(3,3), activation=self.activation, kernel_initializer=self.initializer, kernel_regularizer=self.l2_regularizer))
        ...
        model.add(tf.keras.layers.Dense(128, activation=self.activation,kernel_initializer=self.initializer, kernel_regularizer=self.l2_regularizer))
        model.add(tf.keras.layers.Dropout(0.2))
...
def savePlot(metric, epocs, training, validation):
    ...
    plt.ylabel(metric)
    plt.legend()
    plt.savefig("../plots/grey/" + metric + ".png")
    plt.clf()
